...
def main(csv_file):
    ...
    mapping_json_path = 'mapping_test_5.json'#input("Enter the path of the Mapping JSON file: ") - mapping JSON file
    ...
    has_coordinates = True # True - if coordinates are available in data , False - if not available

    is_wgs84_coordinate = False # True - if coordinates are in WGS 84 , False - if coordintes are in any other coordinate system


    if has_coordinates == True and is_wgs84_coordinate == False:
        x_col = "lon"  # the column name for longitude (x)
        y_col = "lat"  # the column name for latitude (y)
        z_col =  "elev" # # the column name for elevation (z)
        input_epsg = "3035" # the EPSG code of the input CRS (e.g., 3035):", "3035")

        try:
            csv_data = transform_coordinates(
                                csv_data, x_col, y_col, f"EPSG:{input_epsg}", "EPSG:4326")

            print(f"Coordinates transformed to WGS 84!")

            # Add WKT column
            csv_data['wkt_point'] = csv_data.apply(
                lambda row: Point(row[x_col], row[y_col], row[z_col]).wkt, axis=1)

            # Optionally drop x_col, y_col, and z_col
            csv_data.drop(columns=[x_col, y_col, z_col], inplace=True)
            print("Added 'wkt_point' column and dropped original coordinate columns.")
    ...
    # Time dependency
    is_time_dependant_csv = False #input("Is the dataset time dependent? (yes/no): ").lower() == 'yes' - # variable is assigned as true or fales
...
if __name__ == "__main__":
    ...
    # dir_path = r'E:/Geoinformatics TUD/2_LamaH-CE_daily/A_basins_total_upstrm/2_timeseries/daily/Temp_copy/*.*'
    ...
    csv_file_path = "C:/Users/sHaMiLa/Nextcloud/Geoinformatics WHK/Conversion of the RDF/Automatic Convertion to RDF Github/Converted RDFs/Gauge_attributes.csv"
    main((csv_file_path))
